...
class MockHistoryServiceLifecycleSuite
  ...
  test("Service Lifecycle") {
    ...
    assertResult("2", s"batch size in context") {
      sparkCtx.getConf.get(BATCH_SIZE)
    }

    assertResult("true", s"listening flag") {
      sparkCtx.getConf.get(REGISTER_LISTENER)
    }
    ...
    val thrown = intercept[IllegalArgumentException] {
      service.start(YarnExtensionServiceBinding(sparkCtx, applicationId, None))
    }
  ...
  test("ServiceStartArguments1") {
    ...
    intercept[IllegalArgumentException] {
      service.start(YarnExtensionServiceBinding(null, applicationId, None))
    }
  }
  ...
  test("ServiceStartArguments2") {
    val service = new YarnHistoryService()
    intercept[IllegalArgumentException] {
      service.start(YarnExtensionServiceBinding(sparkCtx, null, None))
    }
  }
