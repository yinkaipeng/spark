...
class YarnServiceIntegrationSuite extends AbstractYarnHistoryTests {
  ...
  test("Shutdownhook zero priority") {
    ...
    intercept[IllegalArgumentException] {
      ...
      services.start(sparkCtx, applicationId, null, true, 0)
    }
  }
  ...
  test("Shutdownhook") {
    ...
    services.start(sparkCtx, applicationId, null, true, 10)
...
class SimpleService extends AbstractService("simple") with YarnExtensionService {
   ...
   */
  override def start(sc: SparkContext, appId: ApplicationId, attemptId: ApplicationAttemptId):
      Boolean = {
