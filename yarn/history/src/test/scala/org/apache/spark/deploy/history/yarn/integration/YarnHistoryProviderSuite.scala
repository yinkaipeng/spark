 ...
 */
class YarnHistoryProviderSuite extends AbstractYarnHistoryTests with
  ...
  val app4Running = new ApplicationHistoryInfo(app4AcceptedReport.getApplicationId.toString,
  ...
  */
  ...
  test("Provider Stop+Stop") {
    ...
    val historyProvider = new YarnHistoryProvider(sparkCtx.conf)
   ...
   */
  def assertBecomesCompleted(info: ApplicationHistoryInfo,
   ...
   * @param reports list of applications known by the RM
   */
  def assertUnmodified(info: ApplicationHistoryInfo,
      reports: Map[String, ApplicationReport]): Unit = {
    val (head :: tail) = completeAppsFromYARN(List(info), reports, 5000, 100)
