...
import org.apache.spark.deploy.history.yarn.YarnTimelineUtils
...
private[spark] class YarnHistoryProvider(sparkConf: SparkConf)
  ...
  val _livenessChecksEnabled = !sparkConf.getBoolean(OPTION_DISABLE_YARN_LIVENESS_CHECKS, false)
   ...
   */
  private val _refreshCount = new AtomicLong(0)

  /**
   * Counter of failed refresh operations
   */
  private val _refreshFailedCount = new AtomicLong(0)
  ...
  /**
   * Validate that a time interval is at or above the minimum allowed.
    ...
    try {
      ...
      val nextWindowStart = if (startup) {
        ...
        findStartOfWindow(history) map { app =>
          ...
          val inclusiveWindow = startTime(app) - StartWindowOffset
      ...
      synchronized {
        if (results.succeeded) {
          ...
          _refreshFailedCount.incrementAndGet
        }
      }
      results
    } finally {
  ...
  override def getAppUI(appId: String, attemptId: Option[String]): Option[SparkUI] = {
    ...
    val (foundApp, attempt) = getApplications.lookupAttempt(appId, attemptId)
    val attemptInfo = attempt.getOrElse {
      log.info(s"attempt $attemptId not found under App $appId")
      return None
    }
    ...
    try {
      ...
      ui.setAppName(s"${appInfo.name}")
      logInfo(s"Building Application UI ${ui.appName} under ${ui.basePath}")
    ...
    synchronized {
      ...
      if (detailedInfo) {
        state = state ++ Map(
          ...
          KEY_MIN_REFRESH_INTERVAL -> s"$manualRefreshInterval mS",
          KEY_BACKGROUND_REFRESH_INTERVAL -> s"$backgroundRefreshInterval mS",
          ...
          KEY_REFRESH_IN_PROGRESS -> refreshInProgress.toString,
          KEY_LAST_REFRESH_DURATION -> s"$lastRefreshDuration ms"
        )
      }
      state
    }
  ...
  private[yarn] def listYarnSparkApplications(): Map[String, ApplicationReport] = {
    if (isYarnClientRunning) {
      val reports = yarnClient.getApplications(Set(SPARK_YARN_TYPE).asJava).asScala
   ...
   * @return true if it is non-null & running
   */
  ...
  /**
   * Filter out all incomplete applications
   * that are not in the list of running YARN applications. This picks up
   * on apps which have failed without any notification event.
   * @param apps list of applications
   * @return list of apps which are marked as incomplete but no longer running
   */
  private[yarn] def updateAppsFromYARN(apps: Seq[ApplicationHistoryInfo]):
...
private[spark] object YarnHistoryProvider {
  ...
  val OPTION_DISABLE_YARN_LIVENESS_CHECKS = "spark.history.yarn.disable.liveness.checks"

  val KEY_CURRENT_TIME = "Current Time"
  val KEY_BACKGROUND_REFRESH_INTERVAL = "x-Background refresh interval"
  val KEY_ENTITY_LISTING = "x-Entity Listing URL"
  val KEY_EVENT_FETCH_LIMIT = "x-" + OPTION_EVENT_FETCH_LIMIT
  val KEY_MIN_REFRESH_INTERVAL = "x-manual refresh interval"
  val KEY_TOKEN_RENEWAL = "x-Token Renewed"
  val KEY_REFRESH_IN_PROGRESS = "x-Refresh in Progress"
  val KEY_LAST_REFRESH_DURATION = "x-Last Refresh Duration"
