...
import java.net.{NoRouteToHostException, HttpURLConnection, URL, URLConnection}
...
private[spark] class SpnegoUrlConnector(connConfigurator: ConnectionConfigurator,
  ...
  private def openConnection(url: URL, callerUGI: UserGroupInformation): HttpURLConnection = {
    ...
    if (url.getHost == "0.0.0.0") {
      throw new NoRouteToHostException(s"Cannot connect to an uninitialized host: $url")
    }
