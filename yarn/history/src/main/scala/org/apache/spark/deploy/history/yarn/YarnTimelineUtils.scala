...
private[spark] object YarnTimelineUtils extends Logging {
  ...
  def eventDetails(entity: TimelineEntity): String = {
    ...
    if (events != null) {
      events.map(describeEvent).mkString("\n")
    } else {
      ""
    }
  }
  ...
  def describeEntity(entity: TimelineEntity): String = {
    ...
    try {
      events.map(describeEvent).mkString("\n")
      val otherInfo = entity.getOtherInfo().map {
        case (k, v) => s" $k = $v;"
      }.mkString("\n")
  ...
  def createTimelineEntity(appId: ApplicationId,
    ...
    val entityId = buildEntityId(appId, attemptId, sparkApplicationId, sparkApplicationAttemptId)
    val appIdField = buildApplicationIdField(appId, sparkApplicationId)
    ...
    // build the attempt ID
    val attemptIdField = buildApplicationAttemptIdField(sparkApplicationId,
      sparkApplicationAttemptId)
    entity.addOtherInfo(FIELD_ATTEMPT_ID, attemptIdField)
