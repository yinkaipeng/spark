...
trait YarnExtensionService extends Closeable {
   ...
   * @param attemptId YARN attempt ID if known. Otherwise, null.
   */
  def start(sc: SparkContext, appId: ApplicationId, attemptId: ApplicationAttemptId = null): Boolean
}
...
private[spark] class YarnExtensionServices extends AbstractService("YarnExtensionServices")
  ...
  private var attemptId: ApplicationAttemptId = _
  ...
  def start(context: SparkContext,
      appId: ApplicationId,
      attemptId: ApplicationAttemptId = null,
          ...
          sClass => {
            ...
            try {
              if (clazz.nonEmpty) {
                ...
                // bind this service
                instance.start(sparkContext, appId, attemptId)
